...
Cov <- matrix(rnorm(3*nrow(Y)), ncol=3)
for ( i in 1:nrow(Geno))
{
  f1_obs <- f1
  f2_obs <- f2
  noise <- rnorm(length(f1), sd=  (1/  rsnr ) * var(f1))
  noisy.data [[i]] <-  Geno [i,pos1]*f1_obs +noise +Geno [i,pos2]*f2_obs + Cov [i,1]*f1_cov  + Cov  [i,2]*f2_cov + Cov  [i,3]*f3_cov
...
Est_effect <- adjust_FM_covariate(Y,X=Cov, pos=1:ncol(Y))
plot(Est_effect$fitted_coef[,1],type = "l",
     col="blue",
     lty=2,
     lwd=2)
lines(f1_cov)
...
plot( Est_effect$Y_adjusted, Y-Cov[,1:3]%*%t(cbind(f1_cov,f2_cov,f3_cov)))

...
```{r plot-data-ggsave, echo=FALSE, eval=TRUE, message=FALSE}
...
```{r gwas-by-cpg-ggsave, echo=FALSE, eval=TRUE, message=FALSE}
...
pdat <- transform(pdat,
				  ...
				  lfsr = -log10(lfsr))
p5 <- ggplot(pdat,aes(x = CpG,y = CS,size = lfsr,color = affected)) +
  geom_point(shape = 1) +
  scale_x_continuous(breaks = c(0,seq(4,32,4))) +
  scale_color_manual(values = c("darkblue","darkorange")) +
...
```{r fsusie-affected-cpgs-ggsave, echo=FALSE, eval=TRUE, message=FALSE}
ggsave("demo_affected_cpgs.pdf",
       p5 + scale_x_continuous(breaks = seq(1,32)),
	   height = 1.75,width = 6)
```
...
```{r fsusie-affected-cpgs-TI, fig.height=2, fig.width=5}
pdat <-
  rbind(data.frame(CS = 1,
                   cpg = 1:m,
				   est = fit_TI$fitted_func[[1]],
				   lower = fit_TI$cred_band[[1]]["low",],
				   upper = fit_TI$cred_band[[1]]["up",]),
        data.frame(CS = 2,
		           cpg = 1:m,
				   est = fit_TI$fitted_func[[2]],
				   lower = fit_TI$cred_band[[2]]["low",],
				   upper = fit_TI$cred_band[[2]]["up",]),
		data.frame(CS = 3,
		           cpg = 1:m,
				   est = fit_TI$fitted_func[[3]],
				   lower = fit_TI$cred_band[[3]]["low",],
				   upper = fit_TI$cred_band[[3]]["up",]))
pdat <- transform(pdat,CS = factor(CS))
p6 <- ggplot(pdat,aes(x = cpg,y = est,color = CS)) +
  geom_line(linetype = "dashed") +
  geom_line(mapping = aes(x = cpg,y = upper,color = CS),linewidth = 0.85) +
  geom_line(mapping = aes(x = cpg,y = lower,color = CS),linewidth = 0.85) +
  geom_hline(yintercept = 0,color = "black",linewidth = 0.4) +
  scale_x_continuous(breaks = c(0,seq(4,32,4))) +
  scale_color_manual(values = cs_colors) +
  labs(x = "CpG",y = "change in meth. level") +
  theme_cowplot(font_size = 11)
print(p6)
```
...
```{r fsusie-affected-cpgs-TI-2, fig.height=2, fig.width=5}
pdat2 <- data.frame(CS   = rep(c("L1","L2","L3"),each = m),
                    cpg  = rep(1:m,times = 3),
                    lfsr = unlist(fit$lfsr_func),
					affected = with(pdat,lower > 0 | upper < 0),
                    stringsAsFactors = FALSE)
pdat2 <- transform(pdat2,CS = factor(CS,c("L3","L2","L1")))
p7 <- ggplot(pdat2,aes(x = cpg,y = CS,color = CS,shape = affected)) +
  geom_point() +
  scale_x_continuous(breaks = c(0,seq(4,32,4))) +
  scale_color_manual(values = cs_colors) +
...
```{r fsusie-affected-cpgs-TI-ggsave, echo=FALSE, eval=FALSE, message=FALSE}
```

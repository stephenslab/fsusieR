...
SNPs affect the methylation levels of the same 8 CpGs (near each other
on the chromosome), and the other SNP affects the methylation levels
of 8 different CpGs (also near each other on the chromosome).
...
fit <- susiF(Y,X,L = 3,filter_cs = FALSE,prior = "mixture_normal",
             post_processing = "HMM",verbose = FALSE)
```

ADD TEXT HERE.

```{r show-credible-sets}
fit$cs
```

ADD TEXT HERE.

```{r pip-plot, fig.height=1.5, fig.width=3}

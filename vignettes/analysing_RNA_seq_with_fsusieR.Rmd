...
effect_track <-
    DataTrack(range = GRanges(seqnames = chr,
                          ...
                          groups= group_cred,
              name = "Effect SNP 261",type = "l",   lwd= group_lwd,
                          col = group_colors,
              track.margin = 0.05,cex.title = 0.6,cex.axis = 0.6,
...
```{r}
  ...
  gene_track <- GeneRegionTrack(txdb,genome = "hg38",chromosome = chr,
                                ...
                                rotation.title = 0,cex.title = 0.6,
...
read_counts <-  rbind( colMeans(log1p(Y[x == 1,])),
                       colMeans(log1p(Y[x == 0,])) 
                           )
...
lab_y = "avg. log1p count" 
  data_track <- DataTrack(range = GRanges(seqnames = chr,
                          ...
                          groups = groups,
                          name = lab_y  , type = "p" , #"p",#type = "l",
                          col = geno_colors  ,
                          track.margin = 0.05,cex.title = 0.6,cex.axis = 0.6,
                          col.axis = "black",col.title = "black",
                          fontface = "plain",background.title = "white",
                          fontface.title = 1,cex.legend = 0.6, cex=0.2)
```
...
plotTracks(tracks, from = pos0, to = pos1, sizes = c(  3, 4, 4 ))
```


### Fine mapping SNP effect on RNA seq

The example above is nice, however, this naive approach cannot allow to asses if the effect of SNP 261 4
is real or do to linkage disequilbrium with another SNP.

To solve this problem with developed susiF. It can be run as follow
```{r running_fsusie}
library(fsusieR)
result= susiF(Y=log1p(Y_cor),X=as.matrix(X), L=10)
```



```{r}
plot(result$fitted_func[[1
                         ]])
```

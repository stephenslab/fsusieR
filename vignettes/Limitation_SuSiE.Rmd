---
title: "Illustration limitation SuSiE"
...
# Problem set up 
This vignette illustrates some of the limitations of SuSiE when analyzing molecular traits or traits/responses variable that are functions. We  use a simple example where the effect of a SNP spans over multiple CpG.  We start by fine-mapping the effect of the genotype on each CpG in a  sequentially fashion. Then we show that SuSiE can outputs unconsistent results. Finally we show that fSuSiE can handle this problem and output much more interpretable results.
...
which(effect>0)
```
Once the data is simulated, we can run SuSiE on each CpG. For the sake of simplicity, we only run SuSiE with L=1 as we are aware that there is only one effect in this simualtion. In the same line, we also limit our fine mapping to the region where the effect is non-zero (which we don't know in practice). 
...
```
SuSiE only detected effect in at  CpGs number  88,91,94,100. Whereas the effect of the causak SNP (SNP number  700) spans between CpG 65 to CpG 106.Thus showing the lack of power of SuSiE in this scenario.
...
P04 <- P00+geom_point(x=100,y=effect[100], col="red", shape=21,size=4)+
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1.2),
        axis.text.y=element_blank(),
        axis.ticks.y=element_blank(),
        axis.text.x=element_blank(),

...
L <- 2
lf <- list()
lf[[1]] <- rep(0, 2^lev_res); lf[[1]][20:30] <-  .7
lf[[2]] <- rep(0, 2^lev_res); lf[[2]][50:60] <-  .7
...
for (i in 1:N) {
  ...
  for (l in 1:L) {
    ...
    predictor <- predictor + G[i, true_pos[l]] * lf[[l]] + 0.03
  }
...
```{r}
res <- Pois_fSuSiE(Y = Y, X = X, L = 3 ,maxit_outer=20,
                 post_processing ="HMM" )
...
```{r}
res_2iter <- Pois_fSuSiE(Y = Y, X = X, L = 3, maxit_outer = 2 )

...
#' @param sigma2 finite positive residual variance used to initialize the
#' marginal-regression standard errors. Defaults to one for backward
#' compatibility.
                               ...
                               sigma2=1 )
{
  ...
  if (length(sigma2) != 1L || !is.finite(sigma2) || sigma2 <= 0) {
    stop("`sigma2` must be a finite positive scalar")
  }
  ...
  if(missing(ind_analysis)){
    ...
    temp <- cal_Bhat_Shat(Y, X, sigma2 = sigma2,
                          lowc_wc=lowc_wc
                         )   ## Speed Gain would be good to call directly cal_Bhat_Shat in the ash function

  }else{
    temp <- cal_Bhat_Shat(Y[ind_analysis,], X [ind_analysis,,drop=FALSE], sigma2 = sigma2,
                          lowc_wc=lowc_wc
                         )
  }

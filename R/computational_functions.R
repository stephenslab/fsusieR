...
m_step <- function(L, zeta, indx_lst,init_pi0_w, ...)
  UseMethod("m_step")
...
#' @export
#'
m_step.lik_mixture_normal <- function (L, zeta, indx_lst,init_pi0_w , ...)
{
  ...
  mixsqp_out <- mixsqp(L,
                       w,
                       log = TRUE,
                       x0 = c(init_pi0_w ,rep(1e-30,tlength)), # put starting point close to sparse solution
                       control = list(
                                      eps = 1e-6,
                                      numiter.em = 20,
                                      verbose = FALSE
...
#' @export
#'
m_step.lik_mixture_normal_per_scale <- function(L, zeta, indx_lst,init_pi0_w, ...)
{
...
#'
#' @export
scale_m_step <- function(L,s,zeta, indx_lst,init_pi0_w=1)
{
  w <- rep(zeta,length(indx_lst[[s]] ))
  tlength <- dim(L[[s]])[2]-1
  mixsqp_out <- mixsqp( L[[s]] ,
                        w,
                        x0 = c(init_pi0_w, rep(1e-30,  tlength )),
                        log=TRUE ,
                        control = list(
                                       eps = 1e-6,
                                       numiter.em = 20,

...
fit_hmm <- function (x,sd,
                     ...
                     smooth=TRUE,
                     thresh=0.00001,
                     prefilter=TRUE,maxiter=10,
                     thresh_prefilter=1e-5){
  ...
  for ( t in 1:(length(X)-1)){
    ...
    for(  j in  1:(ncol(P)-1)){
      tt_j_j1 <-c(tt_j_j1, (alpha_hat[t, j]*P[j,j+1]*beta_hat[  t+1,j+1 ]*emit(k=j+1,x=X[t+1], t= t+1 )  )  )
      tt_j1_j  <-c(tt_j_j1, (alpha_hat[t, j]*P[j+1,j ]*beta_hat[  t+1,j  ]*emit(k=j ,x=X[t+1], t= t+1 )  )  )

    }

    if(smooth){
      n_c <-  (sum(tt_z_nz) +sum(tt_nz_z )+ sum(tt_self)+sum(tt_j_j1)+sum(tt_j1_j )  )
    }
    ...
    if( n_c==0){
      ...
      if(smooth){
        list_j_j1[[t]] <- tt_j_j1*0
        list_j1_j[[t]] <- tt_j1_j *0
      }
    }else{
      ...
      if(smooth){
        list_j_j1[[t]] <- tt_j_j1/  n_c
        list_j1_j[[t]] <- tt_j1_j /  n_c
      }
    }
  ...
  P[-1,1]<- nz_z
  if(smooth){
    j_j1  <- apply(do.call( cbind,list_j_j1),1 ,sum) /expect_number_obs_state[-1]
    j1_j  <- apply(do.call( cbind,list_j_j1),1 ,sum) /rev(expect_number_obs_state[-1])
    for(j in 2:(ncol(P)-1 )){
      P[j,(j+1)] <-   j_j1[j]
      P[ (j+1),j] <-   j1_j [j]
    }
